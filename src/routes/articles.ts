...
router.get("/:id", (req: Request, res: Response) => {
  console.log("Inspecting req.query ", JSON.stringify(req.query, null, 2));
  ...
  //sparse field: field[articles]
  const fields = "fields[articles]";
  let requestFields: string[] | null = null;

  if (req.query[fields] && typeof req.query[fields] === "string") {
    requestFields = (req.query[fields] as string).split(",");
  }

  // console.log('Requested fields: ', requestFields);
...
router.post("/", (req: Request, res: Response) => {
  ...
  if (
    ...
    return res.status(400).json({ error: "data from the client is missing" });
  }
